...
- JMX Management and Monitoring via JConsole/VisualVM
- CRITICAL log level as synonym for FATAL
- Configuration Hot Reload with file watching
- SLF4J 2.x ServiceProvider binding
- All features from versions 1.0.4 and earlier
...
String version = Log4Rich.getVersion();           // "1.0.5"
...
### Session 6: Enterprise Extensions (December 23, 2025) - Version 1.0.5

#### Phase 4 Implementation
- **Network Appenders**: Implemented TCP, UDP, and Syslog appenders for centralized logging
  - NetworkAppender base class with retry logic and connection management
  - TCPAppender with auto-reconnect and keep-alive support
  - UDPAppender for fire-and-forget logging
  - SyslogAppender with RFC 3164 compliance and facility/severity mapping
- **JDBC Database Appender**: Log to any JDBC-compatible database
  - Batch insert support for high performance
  - Auto table creation
  - Statistics tracking (messages written, batches, failures)
- **JMX Management**: Full JMX MBean for monitoring and management
  - JMXManager for easy registration/unregistration
  - Log4RichMXBean interface with runtime configuration
  - Statistics access via JConsole/VisualVM
- **CRITICAL Log Level**: Added as synonym for FATAL (priority 600)
  - critical() methods on Logger
  - isCriticalEnabled() check
- **Configuration Hot Reload**: Automatic config file change detection
  - ConfigurationWatcher using Java NIO WatchService
  - 500ms debouncing to avoid multiple reloads
